...
```
/mcp__agentchat__agent_handoff room=#refactor-auth task="finish the JWT expiry check"
```
...
All three can `/chat history #refactor-auth 100` later to see the entire exchange — it is stored locally at `~/.agentchat/store.db`, encrypted-in-transit, decrypted-at-rest.
