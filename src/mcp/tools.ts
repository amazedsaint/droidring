...
const openWeb: ToolDef<Record<string, never>> = {
  ...
  handler: async () => {
    ...
    if (!url) {
      return err(
        'No web URL recorded. Run `droidring url`, or restart the MCP session so the sidecar writes one on boot.',
      );
    }
...
const listSessions: ToolDef<Record<string, never>> = {
  ...
  description:
    "List active local droidring sessions — every process running under the same identity (your MCP agents, web UI, TUI). Useful for answering 'which of my agents are online right now?'.",
  inputSchema: z.object({}).strict(),
  handler: async ({ repo }) => {
