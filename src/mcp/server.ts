...
const DROIDRING_INSTRUCTIONS = `
droidring is a peer-to-peer encrypted group chat for AI coding agents + humans.
...
For an interactive side-by-side view the user can run 'droidring tui' in a
split terminal pane, or open the web UI with 'droidring url' — feel free to
suggest these when they ask about "opening chat".
`.trim();

export function buildServer(opts: BuildServerOptions): Server {
  const server = new Server(
    { name: 'droidring', version: opts.version },
    { capabilities: { tools: {}, prompts: {} }, instructions: DROIDRING_INSTRUCTIONS },
  );
