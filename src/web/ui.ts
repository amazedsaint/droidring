...
<div id="login">
  <form id="login-form" class="card">
    ...
    <div class="help-box">
      ...
      <p>When droidring is launched by Claude Code, Codex, or another MCP
         host, the sign-in URL is logged to <em>that host's</em> log file —
         not shown in the chat. You can always recover it locally:</p>
      <div class="cmd-row">
        <code>droidring url</code>
        <button type="button" class="copy-btn" data-copy="droidring url">Copy</button>
        <small>prints the full URL</small>
      </div>
      <div class="cmd-row">
        <code>cat ~/.droidring/web-token</code>
        <button type="button" class="copy-btn" data-copy="cat ~/.droidring/web-token">Copy</button>
        <small>just the token</small>
      </div>
      <div class="cmd-row">
        <code>droidring doctor</code>
        <button type="button" class="copy-btn" data-copy="droidring doctor">Copy</button>
    ...
    <div class="field">
      ...
      <input id="token-input" type="password" autocomplete="off" spellcheck="false"
             placeholder="paste the output of droidring url or the token itself">
    </div>
...
<div id="app" class="hidden aside-collapsed">
  ...
  <aside id="sidebar">
    <div class="sidebar-head">
      <span class="brand">droidring</span>
...
(function() {
  ...
  // ------- theme -------
  const THEME_KEY = 'droidring_theme';
  ...
  const TOKEN_KEY = 'droidring_token';
  const PENDING_JOIN_KEY = 'droidring_pending_join';
  ...
  // 'droidring url', 'cat ~/.droidring/web-url', or the web-token file.
   ...
   * shell. In a plain browser tab, droidringShell is undefined so these
   * become no-ops. */
  const unread = new Map();
  const shell = typeof window !== 'undefined' ? window.droidringShell : undefined;
  ...
  function updateBadgeAndNotify(wireMsg) {
    ...
    try {
      ...
      const title = room ? room.name : 'droidring';
  ...
  function makeWelcome() {
    ...
    h.textContent = 'Welcome to droidring';
    e.appendChild(h);
    const p = document.createElement('p');
    ...
    for (const t of [
      ...
      '\u{1F464} Humans use this web UI or the terminal (droidring tui).',
  ...
  function composeInvite(roomName, ticket) {
    // Quick-link points at the RECIPIENT's default droidring port (7879),
    ...
    return [
      'You\\'re invited to "' + roomName + '" on droidring',
      '(peer-to-peer, end-to-end encrypted chat).',
      '',
      '── How to join ─────────────────────────',
      '',
      '1. Install droidring (macOS / Linux):',
      '   curl -fsSL https://raw.githubusercontent.com/amazedsaint/droidring/main/install.sh | sh',
      '',
      '2. Start the web UI:',
      '   droidring web',
      '   (it also auto-launches when Claude Code or another MCP client spawns droidring-mcp)',
      '',
      '3. In the UI, click the "join" icon (top-left) and paste this ticket:',
      '',
      '   ' + ticket,
      '',
      'Already running droidring at the default port? One-click join:',
      '   ' + quickLink,
      '',
      'More: https://github.com/amazedsaint/droidring',
    ].join('\\n');
  }
  ...
  $('share-email').addEventListener('click', () => {
    const subject = 'Invite: join "' + currentShareRoom + '" on droidring';
  ...
  $('share-native').addEventListener('click', async () => {
    ...
    const payload = {
      title: 'Join "' + currentShareRoom + '" on droidring',
